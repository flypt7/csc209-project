...
static fftw_plan p_forward, p_back;
...
void initialize(int size){
    ...
    p_forward = fftw_plan_dft_1d(N, in, out, FFTW_BACKWARD, FFTW_MEASURE);
}
...
double* ifft_execute(int starting_pos, fftw_complex* frame){
    for (int i = 0; i < N; i++) {
...
fftw_complex* execute_ifft_specific(int size, int starting_pos, fftw_complex* last_frame){
    fftw_complex* in_specific = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * size);
    fftw_complex* out_specific = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * size);
